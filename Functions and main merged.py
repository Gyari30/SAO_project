...
def SeatingManager(arriving_passenger, seating_matrix, current_time):
    ...
    if seat == 3 or seat == 4:
        ...
        aisle_seating = random.uniform(2,
                                       5)  # Aisle seating time (aisle can be taken regardless of people already sitting in the row
        ...
        if bool(seating_matrix[row - 1][target_aisle - 1]) is True:
            if bool(seating_matrix[row - 1][target_middle - 1]) is True:
                ...
                seating_matrix[row - 1][target_aisle - 1]["timespent"][
                    "standup"] += aisle_seating * 2 + middle_seating * 2 + window_seating
                seating_matrix[row - 1][target_middle - 1]["timespent"][
                    "standup"] += middle_seating * 2 + window_seating
    ...
    arriving_passenger["timespent"]["time_seated"] = current_time[0] + arriving_passenger["timespent"][
        "luggage_stash"] + arriving_passenger["timespent"]["seating_time"]
    arriving_passenger["timespent"]["board_time_cum"] = arriving_passenger["timespent"]["time_seated"] - \
                                                        arriving_passenger["timespent"]["time_entrance"]
...
def BoardingSimulator9000(current_time, passengers, aisle, events, seating_matrix, mu_arrival):
    ...
    if events[action_index]['ID'] == 'Arrival':
        ...
        if bool(aisle[0]):
            ...
            # Note that we are only counting waiting time on the plane. Waiting time in the tube is disregarded
        else:
            events[action_index]['timer'] = random.expovariate(
                1 / mu_arrival)  # Otherwise draw from an exp. distribution with mean 2
        ...
        pass_index = next((index for (index, d) in enumerate(passengers) if d["ID"] == events[action_index]['ID']),
                          None)
...
def SimulationExecutor(strategy, nrows, nseats, l_aisle, size_aisle_seats, size_aisle_begin, mu_arrival):
    # Initialization
    current_time = [
        0]  # Initializing global time. For some reason this cannot simply be an integer, otherwise it does not update
...
def Simulation_analysis(strategies, nrows, nseats, l_aisle, size_aisle_seats, size_aisle_begin, mu_arrival):
    ...
    seating_matrix = SimulationExecutor(strategies[0], nrows, nseats, l_aisle, size_aisle_seats, size_aisle_begin,
                                        mu_arrival)
    ...
    for strat in enumerate(strategies):
        ...
        for i in range(nsims):
            seating_matrix = SimulationExecutor(strat[1], nrows, nseats, l_aisle, size_aisle_seats, size_aisle_begin,
                                                mu_arrival)
            passenger_list = seating_matrix.reshape(-1)
            for key in keys:
                if key not in keys0:
            ...
            time_measures[strat[0]][strat[1]]['Total_boarding_time'].append(
                max(passenger_list, key=lambda x: x['timespent']['time_seated'])['timespent']['time_seated'])
